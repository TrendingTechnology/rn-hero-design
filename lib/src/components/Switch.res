...
@get external getLeftProperty: Style.t => float = "left"
...
let make = (~value=false, ~onValueChange=noop, ~theme=Hero_Theme.default) => {
  ...
  let (animatedOffset, _) = React.useState(() => Animated.Value.create(offset))

  React.useEffect1(_ => {
    open Animated.Value.Timing
    let config = config(~toValue=fromRawValue(offset), ~duration=200.0, ~useNativeDriver=false, ())
